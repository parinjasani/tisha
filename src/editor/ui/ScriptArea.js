...
export default function ScriptArea({ selectedActorId }) {
  ...
  const clickBlock = async (block) => {
    // ✅ If block is "Start on Green Flag" → directly run script
    if (block.category === "start" && block.name === "Start on Green Flag") {
      try {
        await run(); // <-- wait for async run
      } catch (err) {
    ...
    // ✅ If block is in "start" category but not Green Flag → do nothing
    if (block.category === "start") {
      return;
    }

    // ✅ Otherwise open number picker (default behavior)
    setTapBlock(block);
    setPickerOpen(true);
  };
  ...
  async function run() {
    ...
    for (let i = 0; i < actor.scripts.length;) {
      ...
      switch (b.type) {
        ...
        case 'Grow Size':
          for (let k = 0; k < c; k++) {
            dispatch(scaleActor({ actorId: actor.id, scale: 1.2, fromScript: true })); // increase 20%
            await delay(200);
          }
          break;
        ...
        case 'Shrink Size':
          ...
          break;
        case "Reset Size":
          dispatch(resetActorSize({ actorId: selectedActorId, fromScript: true }));
          break;
        
        case "Disappear":
          dispatch(disappearActor({ actorId: selectedActorId }));
          break;

        case "Appear":
          dispatch(reappearActor({ actorId: selectedActorId }));
          break;
